...
{
	CLASS(Cat, MULTIBASE(Furious, Pet)) // multibase must be declared explicitly, no way has been found to do in automatically
...
int main()
{
	...
	Cat alice;
	Animal* animal = &alice;
	cat_class = animal->GetClass(); // gets the same class as above, but via the abstract base
